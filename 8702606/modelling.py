...
obs_table = Table.read("freqs2.dat", format="ascii")
beta_table = Table.read("beta.dat", format="ascii")
freq_table = Table.read("sample_0690-freqs.dat", format="ascii")
...
#    test_omega = test_omega
    ...
    with pm.Model() as model:
        ...
        mu1 = pm.Uniform("mu_omega_1",500, 1000)
        #mu1 = 500
#        if (p_true == 1):
#
#            mu2 = pm.Uniform("mu_omega_2",0, 300)
#        else:
        mu2 = pm.Normal("mu_omega_2",mu = 291.39/2, sigma = 37.56/4)
        #mu2 = 200
        ...
        trace = pm.sample(n_samples, tune=1000, chains=2,cores = 4, target_accept = t, start={"mu_omega_1":800,"mu_omega_2":150,"point":0.06})
        #        trace = pm.sample(10000, tune=2000, chains=2,cores = 4,target_accept = t, start = {"mu_omega_1":525,"mu_omega_2":187,"point":p_true})
    pickle_file_name_trace = str(p_true)+str(s)+str(t)+'_trace_prior.pkl'
    ...
    pickle_file_name_sampled = str(p_true)+str(s)+str(t)+'_pred_trace_prior.pkl'
    ...
    plt.savefig(str(p_true)+str(s)+str(t)+'_corner_prior.png')
    #plot sampled splittings
    plt.figure(figsize=[7.5,4])
    N_samples_post = 1000
    with model:
        pred_samples = pm.sampling.sample_posterior_predictive(trace,samples=N_samples_post,var_names=['l1','l2','omega'])

    pickle_file_name_samples = str(p_true)+str(s)+str(t)+'_pred_samples_prior.pkl'
    ...
    plt.savefig(str(p_true)+str(s)+str(t)+'_splittings_prior.png')
    ...
    if (plot_chains == True):
        ...
        axs[0].plot(trace['mu_omega_1'][:half_n_samples],alpha=0.5)
        axs[0].plot(trace['mu_omega_1'][half_n_samples:],alpha=0.5)
        axs[0].set_ylim(np.min(trace['mu_omega_1']),np.max(trace['mu_omega_1']))
        axs[0].set_ylabel(r'$\Omega_c /2\pi$ (nHz)')

        axs[1].plot(trace['mu_omega_2'][:half_n_samples],alpha=0.5)
        axs[1].plot(trace['mu_omega_2'][half_n_samples:],alpha=0.5)
        axs[1].set_ylim(np.min(trace['mu_omega_2']),np.max(trace['mu_omega_2']))
        axs[1].set_ylabel(r'$\Omega_s /2\pi$ (nHz)')

        axs[2].plot(trace['point'][:half_n_samples],alpha=0.5)
        axs[2].plot(trace['point'][half_n_samples:],alpha=0.5)
        ...
        sample_a = sample[:half_n_samples]
        sample_b = sample[half_n_samples:]
        ...
        corner.corner(sample_b,labels=[r"$\Omega_c/2 \pi$ [nHz]",r"$\Omega_s/2 \pi$ [nHz]", 'p'],
                              ...
                              alpha = 0.5,
                                fig = fig,
                               show_titles=True, title_kwargs={"fontsize": 10});
    plt.savefig(str(p_true)+str(s)+str(t)+'_corners_prior.png')
    # f_out.close()
